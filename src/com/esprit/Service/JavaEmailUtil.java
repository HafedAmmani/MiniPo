...
public class JavaEmailUtil {
    ...
    private static Message prepareMesssage(Session session, String MyAccountEmail, String recepient, int id) {
        try {
            ...
            ResultSet rs1 = ste.executeQuery("SELECT user.username,commande.id, livraison.destination, user.Firstname, user.Lastname, livraison.matriculeL, commande.refC, user.email , livraison.idc, livraison.dateliv FROM user, livraison,commande WHERE user.id=livraison.id AND commande.idcmd=livraison.idc AND livraison.idliv=" + id);
            int idc = -1;
            int idClient = -1;
            String username = "";
            String adresse = "";
            ...
            while (rs1.next()) {
                ...
                adresse = rs1.getString("destination");
                nom = rs1.getString("Firstname");
                prenom = rs1.getString("Lastname");
